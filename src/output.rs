...
impl ResponseTable {
    pub fn from_json(json: serde_json::Value) -> anyhow::Result<ResponseTable> {
        let rows = match json.as_array() {
            None => bail!("Expected an array"),
            Some(array) => array,
        };

        let headers = match rows.get(0) {
            None => bail!("Empty response"),
            Some(row) => match row.as_object() {
                None => bail!("Expected object inside array"),
                Some(row) => row
                    .iter()
                    .map(|(header, _value)| header.to_string())
                    .collect(),
            },
        };
        ...
        let rows = rows
            ...
            .filter_map(|row| match row.as_object() {
                None => None,
                Some(obj) => Some(Value::Array(obj.values().cloned().collect())),
            })
            .collect::<Vec<_>>();
